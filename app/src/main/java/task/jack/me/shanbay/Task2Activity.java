...
/**
 * 1. 用列表的形式展示一组初始图片                                                                    OK
 * 2. 图片显示的逻辑是                              核心代码{@link ImageViewHolder#loadImage(String)}OK
 * a. 先显示本地图片                                                                                OK
 * b. 本地没有图片时，根据相应的url下载图片，下载完毕后，显示已下载的图片，
 * 并且保存在本地。在外部存储上的私有存储目录中创建一个Images文件夹，用来
 * 存放本地图片。(目录:SD卡目录/Android/data/包名/Images )                                           OK
 * c. 无网络或者下载失败时，显示默认图片(res文件)                                                      OK
 * d. 图片下载失败时，增加合理的重试机制                                   Picasso内部有默认的重试机制（3次）
 * 3. 有按钮可以用来把当前的一组图片更新成新的一组图片       {@link ImageViewHolder#refreshImage(String)}OK
 * a. 更新时，优先获取网络图片，并且同样的需要保存在本地(有本地图片，则进行 覆盖)                           OK
 * b. 无网络或者下载失败时，显示本地图片                                                               OK
 * c. 如果没有本地图片，显示默认图片                                                                  OK
 * d. 图片下载失败时，增加合理的重试机制                                                             同上
 * 4. 合理的注释方便代码阅读                                                                         OK
 */
